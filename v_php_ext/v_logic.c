...
u32 v_typeof_interface_idx_IError(u32 sidx) {
	...
	if (sidx == _IError_os__Eof_index) return 166;
	if (sidx == _IError_os__NotExpected_index) return 167;
	if (sidx == _IError_os__FileNotOpenedError_index) return 169;
	if (sidx == _IError_os__SizeOfTypeIs0Error_index) return 170;
	if (sidx == _IError_os__ExecutableNotFoundError_index) return 188;
	return 30;
}
static char * v_typeof_interface_vphp__ITask(u32 sidx) {
...
VV_LOC void main__main(void) {
	...
	string gcc_cmd = builtin__string__plus(builtin__string__plus(builtin__string__plus(builtin__string__plus(builtin__string__plus(_S("gcc -shared -fPIC -DCOMPILE_DL_V_PHP_EXT=1 "), builtin__str_intp(3, _MOV((StrIntpData[]){{_S("-I"), 0xfe10, {.d_s = brew_path}}, {_S("/include -L"), 0xfe10, {.d_s = brew_path}}, {_S("/lib -lcjson "), 0, { .d_c = 0 }}}))), _S("-DcJSON_GetErrorPos=cJSON_GetErrorPtr ")), builtin__str_intp(2, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = php_inc}}, {_S(" v_logic.c php_bridge.c ../vphp/v_bridge.c -o v_php_ext.so "), 0, { .d_c = 0 }}}))), _S("-I../vphp ")), builtin__str_intp(3, _MOV((StrIntpData[]){{_SLIT0, 0xfe10, {.d_s = php_ldflags}}, {_S(" "), 0xfe10, {.d_s = php_libs}}, {_S(" -undefined dynamic_lookup -fvisibility=default"), 0, { .d_c = 0 }}})));
